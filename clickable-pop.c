...
void createLoginWindow(Window fen)
{
  ...
  for (count = 0, i = 30; count < 2; count++, i += height) {
    loginWin[count] = XCreateSimpleWindow(dpy, fen, j, i, (width * 1) - BORDER, (height * 1) - BORDER, BORDER, WhitePixel(dpy, DefaultScreen(dpy)), bgcolor.pixel);
...
void createListWindow()
{
  ...
  for (counterX = 0, i = MARGIN; counterX < own.nb_tr; counterX++, i += height) {
    for (counterY = 0, j = MARGIN; counterY < own.nb_td; counterY++) {
      filles[counterX * own.nb_td + counterY] = XCreateSimpleWindow(dpy, racine, j, i, width - BORDER, height - BORDER, BORDER, WhitePixel(dpy, DefaultScreen(dpy)), bgcolor.pixel);
  ...
  filles[own.nb_tr * own.nb_td] = XCreateSimpleWindow(dpy, racine, MARGIN, MARGIN + height * own.nb_tr, (width - BORDER) * 3 + MARGIN, height - BORDER, BORDER, WhitePixel(dpy, DefaultScreen(dpy)), bgcolor.pixel);
...
void createMainWindow()
{
  ...
  racine = XCreateSimpleWindow(dpy, XRootWindow(dpy, screen), 200, 200, (own.width + 50), (own.height + 50), BORDER, WhitePixel(dpy, DefaultScreen(dpy)), bgcolor.pixel);
  ...
  gc = XCreateGC(dpy, racine, GCFont | GCFunction | GCPlaneMask | GCForeground | GCBackground, &gr_values);
...
void createRetrWindow(int mexId)
{
  ...
  bgcolor.pixel = 25000;
  screen = DefaultScreen(dpy);
  retrWinList[mexId].main = XCreateSimpleWindow(dpy, XRootWindow(dpy, screen), 0, 0, 500, 500, BORDER, WhitePixel(dpy, DefaultScreen(dpy)), bgcolor.pixel);

  //CREATING SUB-RIGHT-WINDOW
  XColor subColor;
  subColor.pixel = 50000;
  retrWinList[mexId].scroll = XCreateSimpleWindow(dpy, retrWin[mexId].main, 450, 0, 50, 500, BORDER, WhitePixel(dpy, DefaultScreen(dpy)), subColor.pixel);

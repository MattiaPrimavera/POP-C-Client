...
void createRetrWindow(int mexId)
{
  ...
  Window retrWin = XCreateSimpleWindow(dpy, XRootWindow(dpy, screen), 0, 0, 500, 500, BORDER, WhitePixel(dpy, DefaultScreen(dpy)), bgcolor.pixel);

  //CREATING SUB-RIGHT-WINDOW
  XColor subColor;
  subColor.pixel = 50000;
  Window retrWinSub = XCreateSimpleWindow(dpy, retrWin, 450, 0, 50, 500, BORDER, WhitePixel(dpy, DefaultScreen(dpy)), subColor.pixel);
